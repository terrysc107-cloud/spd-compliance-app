...
- Do NOT write competency criteria using personality traits ("shows initiative," "is a team player") — observable behaviors only
- Do NOT use a self-assessment alone for any patient-safety-critical skill — direct observation or return demonstration required
- Do NOT document a competency as "competent" without specifying the method and criteria met
- Do NOT skip event-triggered reassessment because the staff member "seems fine" — the trigger exists for regulatory and safety reasons
- Do NOT store competency records only in a supervisor's file — they must be in SQ Track or the facility-approved system

## Wiring

**Called by:** spd-orchestrator (initial competency builds, annual schedules, event-triggered reassessment), spd-educator-agent (competency verification component of training programs), spd-survey-readiness (competency documentation audit)
**Calls:** spd-training-materials (remediation content), spd-educator-agent (scheduling and delivery), spd-outcomes-tracker (gap closure tracking), spd-quality-gate (before any competency document is used as a survey response)
